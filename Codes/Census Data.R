...
##Load Development Tiers
base_url2 = "https://webgis.durhamnc.gov/server/rest/services/PublicServices/Planning/MapServer/5/query?outFields=*&where=1%3D1&f=geojson"
dev_tiers <- st_read(base_url2)%>% st_transform(4326)%>%st_as_sf()

##Combine CNs into surrounding areas
dev_tiers<-dev_tiers%>%
  mutate(TYPE=ifelse(NAME=="Erwin Road CN "|NAME=="Ninth Street CN "|NAME=="Alston Avenue CN ","URBAN",
                     ifelse(NAME=="South Square / MLK CN "|NAME=="Patterson Place CN "|NAME=="Leigh Village CN "|NAME=="RTP North CN "|NAME=="Metro North CN ","SUBURBAN",TYPE)))
...
write.csv(variables_join,"ACS_Variables.csv",row.names = FALSE)
write.csv(acs_variables,"ACS_Variables_All.csv",row.names = FALSE)
...
Durham_Block_Group_Join_ACS_Pre<-get_acs(geography = geoLevel,
  ...
  Durham_Block_Group_Join_ACS_2020<-Durham_Block_Group_Join_ACS_Pre%>% st_as_sf()%>%
    st_transform(4326) %>%
    st_join(dev_tiers, left=TRUE,largest=TRUE, join=st_contains())
  
  
  st_write(Durham_Block_Group_Join_ACS_2020, "Durham_BlockGroup_2020_ACS.geojson")
  ...
  Durham_Block_Group_Join_ACS_Year<-Durham_Block_Group_Join_ACS_Pre%>% st_as_sf()%>%
    st_transform(4326) %>%
    st_join(dev_tiers, left=TRUE,largest=TRUE, join=st_contains())
  
  
  st_write(Durham_Block_Group_Join_ACS_Year, paste("Durham_BlockGroup_",i,"_ACS.geojson",sep=""))
  
  geo_dropped<-Durham_Block_Group_Join_ACS_Year%>%
    st_drop_geometry()
  
  write.csv(geo_dropped,paste("Durham_BlockGroup_",i,"_ACS.csv",sep=""))
