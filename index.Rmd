...
One portion of the analysis will involve comparing characteristics and trends of Durham's urban tier with those of its suburban tier. The urban tier has been upzoned, while the suburban tier has not. These tiers are mapped below. CNs have been combined into the tiers surrounding them.
...
Durham_Block_Group_Join<-st_read("Durham_BlockGroup_2020_ACS.geojson")
Durham_Block_Group_Join_2015<-st_read("Durham_BlockGroup_2015_ACS.geojson")
...
#### ACS 2015 Block Group Level

Since the geometries of block groups change over time, we should also check the 2015 ACS block groups. There are 153 block groups in 2015, compared to 238 in 2020, so one might expect a more coarse fit within the development tiers. However, when grouped by development tier they still provide a good approximation of the areas of interest:

```{r ,echo = FALSE, message=FALSE, warning = FALSE}

factpal <- colorFactor(topo.colors(5), Durham_Block_Join$TYPE)

leaflet(Durham_Block_Group_Join_2015)%>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  addPolygons(color = "#444444", weight = 0.1, smoothFactor = 0.5,
    opacity = 1, fillOpacity = 0.2,
    fillColor = ~factpal(Durham_Block_Group_Join_2015$TYPE),
    highlightOptions = highlightOptions(color = "#444444", weight = 1,
      bringToFront = TRUE),
    
  label = Durham_Block_Group_Join_2015$NAME.x,
  labelOptions = labelOptions(
    style = list("font-weight" = "normal", padding = "3px 8px"),
    textsize = "12px",
    direction = "auto"))%>%
